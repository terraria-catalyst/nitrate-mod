...
internal static class ModifiedTileDrawing
{
    ...
    private static void DrawSingleTile_Inner(bool vanilla, bool solid, int x, int y, FnaVector2 screenPosition, FnaVector2 screenOffset, Tile tile, ushort type)
    {
        ...
        bool flag = /*drawData.tileLight.R >= 1 || drawData.tileLight.G >= 1 || drawData.tileLight.B >= 1 */ true || drawData.tileCache.wall > 0 && (drawData.tileCache.wall == 318 || drawData.tileCache.fullbrightWall());
    ...
    public static void DrawSingleWall(bool vanilla, int i, int j, FnaVector2 screenPosition)
    {
        ...
        if (wall <= 0 || wall == 318 && !wd._shouldShowInvisibleWalls || tile.invisibleWall() && !wd._shouldShowInvisibleWalls)
        {
            return;
        }
