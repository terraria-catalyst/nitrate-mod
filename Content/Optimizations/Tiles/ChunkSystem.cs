...
internal sealed class ChunkSystem : ModSystem
{
    ...
    private void DrawSingleTile(Vector2 screenPosition, Vector2 screenOffset, int tileX, int tileY)
    {
        ...
        if (td._localPlayer.dangerSense && TileDrawing.IsTileDangerous(tileX, tileY, td._localPlayer, drawData.tileCache, drawData.typeCache))
        {
            ...
            if (td._isActiveAndNotPaused && td._rand.Next(30) == 0)
            {
        ...
        if (td._localPlayer.findTreasure && Main.IsTileSpelunkable(tileX, tileY, drawData.typeCache, drawData.tileFrameX, drawData.tileFrameY))
        {
            ...
            if (td._isActiveAndNotPaused && td._rand.Next(60) == 0)
            {
        ...
        if (td._localPlayer.biomeSight)
        {
            ...
            if (Main.IsTileBiomeSightable(tileX, tileY, drawData.typeCache, drawData.tileFrameX, drawData.tileFrameY, ref sightColor))
            {
                ...
                if (td._isActiveAndNotPaused && td._rand.Next(480) == 0)
                {
        ...
        if (drawData.typeCache == 72 && drawData.tileFrameX >= 36)
        {
            ...
            Main.spriteBatch.Draw(TextureAssets.ShroomCap.Value, new Vector2(tileX * 16 - (int)screenPosition.X - 22, tileY * 16 - (int)screenPosition.Y - 26) + screenOffset, new Rectangle(num4 * 62, 0, 60, 42), Lighting.GetColor(tileX, tileY), 0f, TileDrawing._zero, 1f, drawData.tileSpriteEffect, 0f);
        }
            ...
            if (!flag2)
            {
                ...
                else
                {
                    ...
                    for (int i = 0; i < 8; i++)
                    {
                        ...
                        Main.spriteBatch.Draw(drawData.drawTexture, vector + new Vector2(num17, i * num13 + num14), new Rectangle(drawData.tileFrameX + drawData.addFrX + num17, drawData.tileFrameY + drawData.addFrY + num16, num13, num15), color2, 0f, TileDrawing._zero, 1f, drawData.tileSpriteEffect, 0f);
                    }
        ...
        if (highlightTexture != null)
        {
            ...
            int num19 = 0;
            int num20 = 0;
            Main.spriteBatch.Draw(highlightTexture, new Vector2((float)(tileX * 16 - (int)screenPosition.X) - ((float)drawData.tileWidth - 16f) / 2f + (float)num19, tileY * 16 - (int)screenPosition.Y + drawData.tileTop + num20) + screenOffset, empty, highlightColor, 0f, TileDrawing._zero, 1f, drawData.tileSpriteEffect, 0f);
        }
    }
    ...
    public void DrawBasicTile(Vector2 screenPosition, Vector2 screenOffset, int tileX, int tileY, TileDrawInfo drawData, Rectangle normalTileRect, Vector2 normalTilePosition)
    {
        ...
        if (drawData.tileCache.slope() > 0)
        {
            ...
            for (int i = 0; i < 8; i++)
            {
                ...
                Main.spriteBatch.Draw(drawData.drawTexture, normalTilePosition + new Vector2(num6, i * num2 + num3), new Rectangle(drawData.tileFrameX + drawData.addFrX + num6, drawData.tileFrameY + drawData.addFrY + num5, num2, num4), drawData.finalColor, 0f, TileDrawing._zero, 1f, drawData.tileSpriteEffect, 0f);
            }
        ...
        if (!TileID.Sets.Platforms[drawData.typeCache] && !TileID.Sets.IgnoresNearbyHalfbricksWhenDrawn[drawData.typeCache] && td._tileSolid[drawData.typeCache] && !TileID.Sets.NotReallySolid[drawData.typeCache] && !drawData.tileCache.halfBrick())
        {
            ...
            if (tile.halfBrick())
            {
                ...
                if (tile.halfBrick())
                {
                    Main.spriteBatch.Draw(drawData.drawTexture, normalTilePosition + new Vector2(0f, 8f), new Rectangle(drawData.tileFrameX + drawData.addFrX, drawData.addFrY + drawData.tileFrameY + 8, drawData.tileWidth, 8), drawData.finalColor, 0f, TileDrawing._zero, 1f, drawData.tileSpriteEffect, 0f);
                    Rectangle value = new Rectangle(126 + drawData.addFrX, drawData.addFrY, 16, 8);
                    tile = Main.tile[tileX, tileY - 1];
                    if (tile.active())
                    {
                        tile = Main.tile[tileX, tileY - 1];
            ...
            if (tile.halfBrick())
            {
                ...
                Main.spriteBatch.Draw(drawData.drawTexture, normalTilePosition + new Vector2(0f, 8f), new Rectangle(drawData.tileFrameX + drawData.addFrX, drawData.addFrY + drawData.tileFrameY + 8, drawData.tileWidth, 8), drawData.finalColor, 0f, TileDrawing._zero, 1f, drawData.tileSpriteEffect, 0f);
                Main.spriteBatch.Draw(drawData.drawTexture, normalTilePosition + new Vector2(num8, 0f), new Rectangle(drawData.tileFrameX + num8 + drawData.addFrX, drawData.addFrY + drawData.tileFrameY, drawData.tileWidth - num8, drawData.tileHeight), drawData.finalColor, 0f, TileDrawing._zero, 1f, drawData.tileSpriteEffect, 0f);
    ...
    IL_0cc9:
        ...
        goto IL_101e;
    IL_0269:
        Rectangle value3 = new Rectangle(198, drawData.tileFrameY, 16, 16);
