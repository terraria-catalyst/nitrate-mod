	...
	class RemoveBracesFromSingleStatementRewriter : CSharpSyntaxRewriter
	{
		...
		private bool StatementIsSingleLine(StatementSyntax node)
		{
			...
			case BlockSyntax block:
				return block.Statements.Count == 1 && block.GetLeadingTrivia().All(SyntaxUtils.IsWhitespace) && block.GetTrailingTrivia().All(SyntaxUtils.IsWhitespace) && StatementIsSingleLine(block.Statements[0]);
		...
		{
			if (method.Body != null && StatementIsSingleLine(method.Body) && method.Body.DescendantTrivia().All(SyntaxUtils.IsWhitespace) && method.Body.Statements[0] is ReturnStatementSyntax returnStatement && returnStatement.Expression != null)
			{
				method = method
					.WithBody(null)
